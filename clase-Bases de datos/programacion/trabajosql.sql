...
SELECT p.nombre, p.precio INTO nombre, precio FROM productos p WHERE p.id= NEW.producto;

SET NEW.nombreProducto = nombre, NEW.precio=precio, NEW.total= NEW.cantidad* precio;
...
/*UPDATE factura f 
  ...
  update factura f 
    join (select l.factura,sum(l.total*l.ivagrupo) ivaCalculado, sum(l.total) totalCalculado 
      from FROM lineas l GROUP BY l.factura) c1 ON c1.factura=f.id
        SET f.total=c1.totalCalculado, iva=ivaCalculado;
      */

UPDATE factura f JOIN (
  SELECT l.factura,SUM(l.total) AS totalCalculado FROM lineas l GROUP BY l.factura) c1 ON c1.factura=f.id
SET f.total=c1.totalCalculado;

UPDATE factura JOIN (
SELECT l.factura,SUM(l.total*p.ivaGrupo) ivaCalculado FROM lineas l JOIN productos p ON p.id=l.producto GROUP BY l.factura) c1 ON c1.factura=id
  set iva=c1.ivaCalculado,
  totalIva=total+iva;
